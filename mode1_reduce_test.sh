#!/bin/bash
# Mode1 Reduce Test Script
# 1-2-4 拓扑: 1 Spine + 2 Leaf + 4 Host
#
# 用法: ./mode1_reduce_test.sh [COUNT] [ROOT_RANK]
# 示例:
#   ./mode1_reduce_test.sh              # 默认测试 1024 元素，root=0
#   ./mode1_reduce_test.sh 1048576 2    # 测试 4MB，root=2
...
cleanup() {
    ...
    # 收集日志
    mkdir -p ./logs/mode1_reduce
    ssh $SPINE_HOST "cat /root/switch_mode1.log 2>/dev/null" > ./logs/mode1_reduce/spine.log
    ssh $LEAF1_HOST "cat /root/switch_mode1.log 2>/dev/null" > ./logs/mode1_reduce/leaf1.log
    ssh $LEAF2_HOST "cat /root/switch_mode1.log 2>/dev/null" > ./logs/mode1_reduce/leaf2.log
    echo "Logs saved to ./logs/mode1_reduce/"
}

trap cleanup EXIT INT TERM

echo "=== Mode1 Reduce Test ==="
...
cmake .. > /dev/null 2>&1
make switch_mode1 host_reduce_mode1 -j4 > /dev/null 2>&1
if [ $? -ne 0 ]; then
    echo "Build failed!"
    exit 1
fi
...
# 启动 Hosts
echo ""
echo "=== Starting Hosts (Reduce test, root=$ROOT_RANK) ==="

# pku1 (rank 0)
ssh pku1 "cd /root && ./host_reduce_mode1 $LEAF1_DATA_IP $SWITCH_PORT 0 $WORLD_SIZE $ROOT_RANK $COUNT" &
PID1=$!

# pku2 (rank 1)
ssh pku2 "cd /root && ./host_reduce_mode1 $LEAF1_DATA_IP $SWITCH_PORT 1 $WORLD_SIZE $ROOT_RANK $COUNT" &
PID2=$!

# pku3 (rank 2)
ssh pku3 "cd /root && ./host_reduce_mode1 $LEAF2_DATA_IP $SWITCH_PORT 2 $WORLD_SIZE $ROOT_RANK $COUNT" &
PID3=$!

# pku4 (rank 3)
ssh pku4 "cd /root && ./host_reduce_mode1 $LEAF2_DATA_IP $SWITCH_PORT 3 $WORLD_SIZE $ROOT_RANK $COUNT" &
PID4=$!
