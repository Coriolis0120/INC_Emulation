...
struct inccl_group *inccl_group_create(int world_size, int rank, const char *master_ip){
    ...
    if (group == NULL) {
        perror("Failed to allocate memory for inccl_group");
        return NULL;
    }
    printf("[DEBUG] inccl_group allocated successfully (%zu bytes)\n", sizeof(struct inccl_group));
    fflush(stdout);
...
struct inccl_communicator *inccl_communicator_create(struct inccl_group *group, uint32_t size){
    ...
    if (comm == NULL) {
        perror("Failed to allocate memory for inccl_communicator");
        return NULL;
    }
    printf("[DEBUG] inccl_communicator allocated successfully (%zu bytes)\n", sizeof(struct inccl_communicator));
    fflush(stdout);
    ...
    // 确保至少有16个segment，避免小数据量时segment_num为0导致崩溃
    int segment_num = size / 1024 / 4;
    if (segment_num < 16) {
        segment_num = 16;
    }
    printf("[DEBUG] segment_num: %d (size=%u bytes)\n", segment_num, size);

    // 缓冲区大小需要支持滑动窗口
    // 滑动窗口需要 SLIDING_WINDOW_SIZE 个消息的缓冲区
    uint32_t window_buf_size = (uint32_t)SLIDING_WINDOW_SIZE * MESSAGE_SIZE;
    // 取请求大小和滑动窗口大小的较大值
    comm->payload_buf_size = (size > window_buf_size) ? size : window_buf_size;
    printf("[DEBUG] payload_buf_size: %u bytes (requested size=%u)\n", comm->payload_buf_size, size);

    // 创建Completion Queue（完成队列）：用于通知RDMA操作完成
    // 使用固定滑动窗口大小，CQ 需要容纳 send + recv 完成事件
    int cq_size = SLIDING_WINDOW_SIZE * 2;
    if (cq_size > 32767) cq_size = 32767;
    comm->cq = ibv_create_cq(group->local_ib_ctx, cq_size, NULL, NULL, 0);
    ...
    if (comm->send_payload == NULL || comm->receive_payload == NULL) {
        perror("Failed to allocate memory for payload buffers");
        free(comm->send_payload);
        free(comm->receive_payload);
        free(comm);
        return NULL;
    }
    printf("[DEBUG] Payload buffers allocated: send=%p, recv=%p, size=%u bytes each\n",
           (void*)comm->send_payload, (void*)comm->receive_payload, comm->payload_buf_size);
    fflush(stdout);
    ...
    // 使用固定滑动窗口大小
    int max_wr = SLIDING_WINDOW_SIZE < 16384 ? SLIDING_WINDOW_SIZE : 16384;
    ...
    attr.rnr_retry = 7;  // RNR重试次数（7表示无限重试）
...
void inccl_allreduce_sendrecv(struct inccl_communicator *comm, int32_t* src_data, uint32_t len, int32_t* dst_data) {
    ...
    int posted_recv = 0;  // 已 post 的 receive WR 数量
        ...
        if(result > 0) {  // 有完成事件
            ...
            for(int i = 0; i < result; i++){
                ...
                if(tmp->status==IBV_WC_SUCCESS && (tmp->opcode==IBV_WC_RECV || tmp->opcode==IBV_WC_RECV_RDMA_WITH_IMM)) {
                    ...
                    // 补充 post receive WR（滑动窗口：每收到一个就补充一个）
                    if(posted_recv < message_num) {
                        int slot = posted_recv % window_size;  // 使用模运算复用缓冲区槽位
                        memset(&receive_sge, 0, sizeof(receive_sge));
                        receive_sge.addr = (uintptr_t)(comm->receive_payload + slot * PAYLOAD_COUNT * sizeof(int32_t));
                        receive_sge.length = PAYLOAD_COUNT * sizeof(int32_t);
                        receive_sge.lkey = comm->mr_receive_payload->lkey;

                        memset(&rr, 0, sizeof(rr));
                        rr.next = NULL;
                        rr.wr_id = posted_recv;  // wr_id 保持消息索引
...
void inccl_allreduce_write(struct inccl_communicator *comm, int32_t* src_data, uint32_t len, int32_t* dst_data) {
    ...
    while(receive_num != message_num) {
        ...
        uint64_t elapsed_us = (current_time.tv_sec - start_time.tv_sec) * 1000000 +
                              (current_time.tv_usec - start_time.tv_usec);
        if(elapsed_us > POLL_TIMEOUT_US) {
            printf("[AllReduce WRITE] TIMEOUT! receive_num=%d/%d\n", receive_num, message_num);
            fflush(stdout);
            free(wc);
            return;
        }
        if(elapsed_us - last_warn_time > POLL_WARN_INTERVAL) {
            printf("[AllReduce WRITE] Warning: waiting %lu us, receive_num=%d/%d\n",
                   elapsed_us, receive_num, message_num);
            fflush(stdout);
            last_warn_time = elapsed_us;
        }
...
void inccl_reduce_sendrecv(struct inccl_communicator *comm, int32_t* src_data, uint32_t len, int32_t* dst_data, int root_rank) {
    ...
    if (is_root) {
        // 初始 receive WR 数量不能超过 window_size，否则会导致缓冲区槽位冲突
        for(int i = 0; i < window_size; i++) {
            memset(&receive_sge, 0, sizeof(receive_sge));
            ...
            int ret = ibv_post_recv(comm->qp, &rr, &receive_bad_wr);
            if (i < 5 || i == window_size - 1) {
                printf("Reduce root: post recv %d ret %d\n", i, ret);
                fflush(stdout);
            }
            posted_recv++;
        }
        printf("Reduce root: posted %d initial receive WRs\n", posted_recv);
        fflush(stdout);
    }
    ...
    if (is_root) {
        ...
        while(receive_num != message_num) {
            ...
            uint64_t elapsed_us = (current_time.tv_sec - start_time.tv_sec) * 1000000 +
                                  (current_time.tv_usec - start_time.tv_usec);
            if(elapsed_us > POLL_TIMEOUT_US) {
                printf("[Reduce] TIMEOUT! receive_num=%d/%d\n", receive_num, message_num);
                fflush(stdout);
                free(wc);
                return;
            }
            if(elapsed_us - last_warn_time > POLL_WARN_INTERVAL) {
                printf("[Reduce] Warning: waiting %lu us, receive_num=%d/%d\n",
                       elapsed_us, receive_num, message_num);
                fflush(stdout);
                last_warn_time = elapsed_us;
            }
            ...
            if(result > 0) {
                for(int i = 0; i < result; i++){
                    ...
                    if(tmp->status==IBV_WC_SUCCESS && (tmp->opcode==IBV_WC_RECV || tmp->opcode==IBV_WC_RECV_RDMA_WITH_IMM)) {
                        ...
                        // 滑动窗口：补充 receive WR
                        if(posted_recv < message_num) {
                            int slot = posted_recv % window_size;
                            memset(&receive_sge, 0, sizeof(receive_sge));
                            receive_sge.addr = (uintptr_t)(comm->receive_payload + slot * PAYLOAD_COUNT * sizeof(int32_t));
                            receive_sge.length = PAYLOAD_COUNT * sizeof(int32_t);
                            receive_sge.lkey = comm->mr_receive_payload->lkey;
...
void inccl_reduce_write(struct inccl_communicator *comm, int32_t* src_data, uint32_t len, int32_t* dst_data, int root_rank) {
    ...
    for(int i = 0; i < comm->window_size/(MESSAGE_SIZE) && i < message_num; i++) {
        ...
        wr.send_flags = IBV_SEND_SIGNALED;
        wr.wr.rdma.remote_addr = (uintptr_t)(comm->receive_payload + i * PAYLOAD_COUNT * sizeof(int32_t));
                ...
                for(int i = 0; i < result; i++){
                    ...
                    if(tmp->status==IBV_WC_SUCCESS && tmp->opcode==IBV_WC_RECV) {
                        ...
                        if(send_num < message_num) {
                            ...
                            wr.opcode = IBV_WR_RDMA_WRITE;
                            wr.send_flags = IBV_SEND_SIGNALED;
                            wr.wr.rdma.remote_addr = (uintptr_t)(comm->receive_payload + send_num * PAYLOAD_COUNT * sizeof(int32_t));
                            wr.wr.rdma.rkey = comm->mr_receive_payload->rkey;
...
void inccl_broadcast_sendrecv(struct inccl_communicator *comm, int32_t* data, uint32_t len, int root_rank) {
    ...
    if (is_root) {
        ...
        printf("Broadcast non-root: posted %d initial receive WRs\n", posted_recv);
        fflush(stdout);
        ...
        while(receive_num < message_num) {
            ...
            uint64_t elapsed_us = (current_time.tv_sec - start_time.tv_sec) * 1000000 +
                                  (current_time.tv_usec - start_time.tv_usec);
            if(elapsed_us > POLL_TIMEOUT_US) {
                printf("[Broadcast] TIMEOUT! receive_num=%d/%d\n", receive_num, message_num);
                fflush(stdout);
                free(wc);
                return;
            }
            if(elapsed_us - last_warn_time > POLL_WARN_INTERVAL) {
                printf("[Broadcast] Warning: waiting %lu us, receive_num=%d/%d\n",
                       elapsed_us, receive_num, message_num);
                fflush(stdout);
                last_warn_time = elapsed_us;
            }
            ...
            if(result > 0) {
                for(int i = 0; i < result; i++){
                    ...
                    if(tmp->status==IBV_WC_SUCCESS && (tmp->opcode==IBV_WC_RECV || tmp->opcode==IBV_WC_RECV_RDMA_WITH_IMM)) {
                        uint64_t idx = tmp->wr_id;  // 消息索引
                        int slot = idx % window_size;  // 缓冲区槽位（循环使用）
                        int *pack = (int *)(comm->receive_payload + slot * PAYLOAD_COUNT * sizeof(int32_t));
