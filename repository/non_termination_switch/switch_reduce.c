...
static void send_roce_data(uint32_t conn_id, metadata_t *meta_p){
    ...
    for(int i = 0; i < PAYLOAD_LEN / sizeof(int); i++){
...
static void broadcast_roce_ack(metadata_t *meta_p){
...
void pipeline(metadata_t *meta_p, const struct pcap_pkthdr *pkthdr, const uint8_t *packet) {
    ...
    if(meta_p->type == PACKET_TYPE_ACK){
        ...
        broadcast_roce_ack(meta_p);

        printf("[PIPELINE] ACK: broadcast ACK, PSN=%u\n", meta_p->psn);
        fflush(stdout);
        ...
        if(ctx->data_epsn[meta_p->ingress_conn] != meta_p->psn){
            if((ctx->arrival_state[idx] & MASK_CONN(meta_p->ingress_conn)) == 0) { return; } // 超前
            else if(ctx->arrival_state[idx] == ctx->bitmap_mask){ // 重传，聚合完成
        ...
        // in order packet
        assert((ctx->arrival_state[idx] & MASK_CONN(meta_p->ingress_conn)) == 0);
